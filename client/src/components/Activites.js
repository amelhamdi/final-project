...
function Activites({ping,setping}) {
  ...
  const handleShow = () => setShow(true);
    ...
    const [event, setevent] = useState("club")
...
<div class="content">
  ...
  <div> <center><img src={serviceimage} alt="" /> </center></div>
    ...
    <div id="sidebar">
        <h3>les bonnes raisons pour nous choisir</h3>
        <ul>
      ...
      <ul>
      {services?.filter((el)=>el.cathegorie=="club").map((el)=><ServiceCard service={el} ping={ping} setping={setping}/>)}
        
        
      </ul>
      

        <h1> <center>Nos Evenements</center></h1>
      <ul>
      {services?.filter((el)=>el.cathegorie=="evenement").map((el)=><ServiceCard service={el} ping={ping} setping={setping}/>)}
